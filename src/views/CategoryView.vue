...
const idRoute = computed(() => router.params.id);
const pagePaging = reactive({
  page: 0,
  pageSize: 8,
  categoryId: idRoute.value,
});
...
watch(idRoute, async (value) => {
  pagePaging.categoryId = value;
  const resProduct = await getProductByCategory(pagePaging);
